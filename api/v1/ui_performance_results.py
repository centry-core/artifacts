...
class API(Resource):
    ...
    def get(self, result_id: int):
        test_data = self.module.context.rpc_manager.call.ui_results_or_404(run_id=result_id)
        project = self.module.context.rpc_manager.call.project_get_or_404(project_id=test_data["project_id"])
        s3_settings = test_data.get("test_config", {}).get(
        ...
        files = []
        log_file_name = f'{test_data["uid"]}.log'
