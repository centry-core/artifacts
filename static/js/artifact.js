$(document).on('vue_init', () => {
    ...
    $("#artifact-list").bootstrapTable('refresh', {url: `/api/v1/artifacts/artifacts/${getSelectedProjectId()}/${row.name}`});
    })
})
...
function _uploadFile(file) {
    ...
    $.ajax({
        url: `/api/v1/artifacts/artifacts/${getSelectedProjectId()}/${$("#bucket-name").text()}`,
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
...
function deleteBucket(event) {
    $("#buckets-list").bootstrapTable('getSelections').forEach(item => {
        $.ajax({
            url: `/api/v1/artifacts/buckets/${getSelectedProjectId()}?name=${item["name"]}`,
            type: 'DELETE',
            success: function (result) {
                $("#buckets-list").bootstrapTable('refresh');
            }
...
function createBucket() {
    ...
    $.ajax({
        url: `/api/v1/artifacts/buckets/${getSelectedProjectId()}`,
        type: 'POST',
        dataType: 'json',
...
function deleteFiles(ev) {
    var url = `/api/v1/artifacts/artifacts/${getSelectedProjectId()}/${$("#bucket-name").text()}?`
